...
def PACScore(model, transform, images, candidates, device, w=2.0):
    ...
    print("augumented image len", len(images))
    candidates = extract_all_captions(candidates, model, device)

    images = images / np.sqrt(np.sum(images ** 2, axis=1, keepdims=True))    # (16992, 512)
    candidates = candidates / np.sqrt(np.sum(candidates ** 2, axis=1, keepdims=True))    # (16992, 512)
...
def DAScore(model, transform, images, candidates, device, w=2.0):
    '''
    compute the unreferenced DA score.
    '''
    len_candidates = [len(c.split()) for c in candidates] 
    if isinstance(images, list):
        # extracting image features
...
def ClipScore(model, transform, images, candidates, device, w=2.0):
    '''
    compute the unreferenced PAC score.
    '''
    # metric = CLIPScore(model_name_or_path="openai/clip-vit-base-patch16")
    # scores = metric(images, candidates)

    # import pdb; pdb.set_trace()
    # for i in range(len(images)):

    #     image = images[i].unsqueeze(0)
    #     clip_text = clip.tokenize(candidates[i]).to(device)
    ...
    candidates /= candidates.norm(dim=-1, keepdim=True)

    images = images.numpy()
    candidates = candidates.numpy()
    per = np.sum(100.0 * images * candidates, axis=1)

    per = per.numpy()
    per = similarity = (100.0 * image_features @ text_features.T).softmax(dim=-1)
    return np.mean(per), per, None, len_candidates
