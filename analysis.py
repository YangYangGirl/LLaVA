...
for idx in range(0, len(llava_concise), 3):
    ...
    gts_i = gpt_simplify[idx//3]['simplify_text']#.split(' ')
    gen_i = llava_concise[idx]['text']#.split(' ')
    ...
    print(all_scores_metrics)
    import pdb; pdb.set_trace()

    # text = clip.tokenize(gts).to(device)
    # # llava_concise[idx]['clip_score'] = clip_score
